 ...
 * requests (a self-hosted provider — 'local' for Ollama, 'lmstudio' for LM
 * Studio — means nothing but "an endpoint I name"), and the SSRF guard in front
 * of those requests allows loopback and LAN on purpose so a self-hosted server
 * works. Only whoever runs the instance can judge what is reachable from it.
 *
